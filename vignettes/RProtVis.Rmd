...
Authors: [Laurent Gatto](http://proteome.sysbiol.cam.ac.uk/lgatto/),
...
Other non-Bioconductor packages are described in the `r Biocpkg("RforProteomics")`
...
**TODO**: provide examples for the above functions that are not
illustrated in other sections.

### Scatter plots

See the MA and volcano plot examples.

Introduce the `type` arg, default is `p` for points. Other important
types are `l` for lines and `h` for *histogram* (see below).
...
See also the `r CRANpkg("beanplot")` package.
...
See also `r CRANpkg("gplots")`'s `heatmap.2` function and the `r Biocpkg("Heatplus")` Bioconductor package.

The `r CRANpkg("corrplot")` package for correlation matrices.
