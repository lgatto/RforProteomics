...
Since the package is not yet into Bioconductor, it is not yet possible to use its automatice installation and update framework. An ad hoc script is however available to facilitate installation of \Rpackage{RforProteomics} and all dependencies. To install, simple open \R and type 

<<eval=FALSE>>=
source("https://raw.github.com/lgatto/RforProteomics/master/inst/scripts/installR4P.R")
@ 

This \R script is part of the package itself. It will install missing dependencies and then \Rpackage{RforProteomics}, which can then be loaded with
...
rnwfile <- file <- dir(system.file(package="RforProteomics", dir="doc"),
                       full.name = TRUE,
                       pattern = "Rnw$")
...
\begin{figure}[!ht]
<<spikes, dev='pdf', echo=TRUE, fig.width=7, fig.height=5.5, fig.keep='all', out.width='.7\\linewidth'>>=
...
\begin{figure}[!ht]
<<plotmzdelta, dev='pdf', echo=TRUE, fig.width=6.5, fig.height=5.5, fig.keep='last', out.width='7.\\linewidth', cache=TRUE>>=
...
inc99 <- IsotopicDistributionN("YEVQGEVFTKPQLWP", .99)

par(mfrow = c(4,3))
for (i in 1:length(inc))
  plot(inc[[i]][, c(1, 3)], xlim = c(1823, 1848), 
       type = "h", 
       main = paste0("15N incorporation ", incrate[i]))
plot(inc99[, c(1, 3)], xlim = c(1823, 1848), 
     type = "h", 
     main = paste0("15N incorporation 0.99"))
@ 
%% $
...
\begin{figure}[!ht]
<<ibplot, dev='pdf', echo=TRUE, fig.width=10, fig.height=5, fig.keep='last', out.width='.9\\linewidth'>>=
par(mfrow = c(1,2))
maplot(x,
