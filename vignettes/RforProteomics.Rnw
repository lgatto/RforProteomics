...
\title{Using \R and Bioconductor to analyse proteomics data.}
...
General \R ressources.

Getting help: mailing lists, post form, posting guide, session info, make reproducible examples.
...
##pdf("orgmassspecr-compare-spectra.pdf")
sim <- SpectrumSimilarity(as(simplottest[[1]], "data.frame"),
                          ...
                          b = 25)
abline(v = FragmentPeptide("TAGIQIVADDLTVTNPK")[, 7], lty = "dotted")
##dev.off()
sim
...
eno <- download.file("http://www.uniprot.org/uniprot/P00924.fasta", destfile = "Figures/P00924.fasta")
eno <- paste(readLines("Figures/P00924.fasta")[-1], collapse = "")
