...
A recent addition to \Bioconductor{} is the \Biocpkg{rTANDEM} package,
that provides a direct interface to the X!Tandem software
\cite{Craig2004}.  A typical \Biocpkg{rTANDEM} pipeline comprises
...
Using example code/data from the \Biocpkg{rTANDEM} vignette/package,
