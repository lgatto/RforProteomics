...
later time using the \Rpackage{mzID} package (see section~\ref{subsec:mzID} on 
...
through \R. \Rpackage{MSGFgui} is currently available on
GitHub\footnote{\url{https://github.com/thomasp85/MSGFgui}} and will
become available in Bioconductor in the the release. Besides
facilitating MS-GF+ analyses, which is arguably just as easy from the
command line, it provides an intuitive way to investigate and evaluate
