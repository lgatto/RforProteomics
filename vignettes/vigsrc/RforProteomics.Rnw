...
\author{Laurent Gatto\footnote{\email{lg390@cam.ac.uk}}, Sebastian Gibb}
...
\subsection{The \Biocpkg{mzR} package}
...
\texttt{mzML} data is taken from the \Biocexptpkg{msdata} data package.
...
MS-GF+ (using \Rpackage{MSGFplus} or in other ways) can easily be imported into 
