...
\begin{abstract}
  ...
  aims at being a general overview useful for new users who wish to
  explore the \R environment and programming language for the analysis
  of proteomics data.
\end{abstract}
...
avaiable on arXiv\footnote{\url{http://arxiv.org/abs/1305.6559}}.
...
To install all dependencies (78 packages) and reproduce the code in
the vignette, replace the last line in the code chunk above with:)
...
<<gist4493747, echo=FALSE, warning=FALSE, cache=FALSE>>=
