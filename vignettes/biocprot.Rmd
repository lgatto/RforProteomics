...
Bioconductor version `r as.character(BiocManager::version())`.
...
## the following packages - remove from the list
## ppkgs <- setdiff(ppkgs, "omicRexposome") ## works now, Oct 2019
ppkgs <- setdiff(ppkgs, "sparsenetgls")
```
...
developer/contributors over time. See `biocpkg::contribs` for some
code to achieve this.], these figures indicate an increased interest
in computational proteomics community.
