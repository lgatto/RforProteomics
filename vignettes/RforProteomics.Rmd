...
* `r Biocpkg("biomaRt")` performs on-line requests using the
  `[curl](<http://curl.haxx.se/) infrastructure. On Debian-based
...
`readMSData2` function is now available to access the raw
...
```{r n15, fig.cap = "Isotopic envelope for the `YEVQGEVFTKPQLWP` peptide at different N15 incorporation rates "}
...
The `r Biocpkg("isobar")` package [@Breitwieser2011] provides methods for the statistical
analysis of isobarically tagged MS2 experiments.

```{r isobar, cache=TRUE, tidy=FALSE}
library("isobar")
...
```{r ibplot, fig.caption = "Result from the `r Biocpkg('isobar')` pipeline."}
maplot(x,
...
```{r xtabpkg, echo=FALSE, message=FALSE}
library("xtable")
```

```{r protstab, echo=FALSE, results='asis'}
t1 <- xtable(pkTab[["Proteomics"]],
             caption = "Packages available under the \`Proteomics` \*biocViews* category.",
             table.placement = "thb",
             label = "tab:prot")
...
```{r mstab, echo=FALSE, results='asis'}
t2 <- xtable(pkTab[["MassSpectrometry"]],
             caption = "Packages available under the \`MassSpectrometry` \*biocViews* category.",
             table.placement = "thb",
             label = "tab:ms")
...
```{r msdatatab, echo=FALSE, results='asis'}
t3 <- xtable(pkTab[["MassSpectrometryData"]],
             caption = "Experimental Packages available under the \`MassSpectrometryData` \*biocViews* category.",
             table.placement = "thb",
             label = "tab:msdata")
...
`proteomicsPackages`, `massSpectrometryPackages`
and `massSpectrometryDataPackages` functions. The respective
