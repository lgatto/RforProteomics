...
Following Bioconductor 2.12 the `r Biocpkg("rTANDEM")` package
provides the means to run the popular X!Tandem software
[@Craig2004]. For details, please refer to the package vignette
available with `vignette("rTANDEM")`.
...
The `r Biocpkg("MQmetrics")`package generates a pdf report containing several
metrics regarding the sample preparation and LC-MS/MS runs. As input, it takes 
the MaxQuant output tables. It is designed for people not so well versed
in R, so it is very user-friendly.
