...
The function to read mzTab data version 0.9 has been deprecated. We
are going to load it by hand and create a `SummarizedExperiment`. See
`?readSummarizedExperiment` from the QFeatures package for details.
...
Here, we convert the `SummarizedExperiment` to an `MSnSet` to fit the
