...
\begin{description}
  ...
  \item[\Rpackage{MSGFplus}] Is based on the MS-GF+ java program and thus 
...
\subsection{X! Tandem}
Following Bioconductor 2.12 the \Rpackage{rTANDEM} package provides the means
to run the popular X! Tandem software \cite{Craig2004}.

Using example code/data from the \Rpackage{rTANDEM} vignette/package, the 
...
\subsection{MS-GF+}
With the release of Bioconductor 3.0 the \Rpackage{MSGFplus} package has 
...
\subsubsection{Import and analyse results}
By default MSGFplus imports the results automatically using \Rpackage{mzID}. If
...
later time using the \Rpackage{mzID} package (see below).

\subsubsection{Running MS-GF+ through a GUI}
\Rpackage{MSGFplus} comes with a sister package, \Rpackage{MSGFgui}, which
provide a graphic interface to setting up and running MS-GF+ through R. Besides
facilitating MS-GF+ analyses, which is arguably just as easy from the command 
line, it provides an intuitive way to investigate and evaluate the resulting 
identification data. 

\begin{figure}[!ht]
\includegraphics[width=\textwidth]{MSGFgui}
\caption{A screenshot of MSGFgui}
\label{fig:MSGFgui}
\end{figure}

\ref{fig:MSGFgui} shows an example of using \Rpackage{MSGFgui}. It is possible
to gradually drill down in the results starting from the protein level and
ending at the raw spectrum level. mzIdentML files already created with MS-GF+ 
(using \Rpackage{MSGFplus} or in other ways) can easily be imported into the gui
to take advantage of the visualisation features, and results can be exported as
either rds (for R), xlsx (for excel) or txt (for everything else) files.

\subsection{Importing identification results}
In the same way the mzML format has been proposed as the default open standard 
for storing raw mass spectrometry data, the mzIdentML format has been developed
to store identification data in a structured manner. Most current search tools
supports outputting to this format (see the implementation 
homepage\footnote{\url{http://www.psidev.info/tools-implementing-mzidentml}} for
a list of all software), even though it is not necessarily the default. Support 
for mzIdentML reading was added to Bioconductor 2.14 with the \Rpackage{mzID} 
package. \Rpackage{mzID} supports both reading af single files and multiple 
files in parallel using the \Rpackage{foreach} package.
...
The imported data contains information regarding the software and parameters
used for the search as well as the database searched against in addition to the
actual search results. The search results are stored in a manner that reflects
the mzIdentML structure, so instead of having a flat table with a row per match
several interconnected tables exists, all related to their own type of 
information. These tables can be accessed with their respective getter 
functions:

<<mzid2>>=
pep <- peptides(results)
head(pep)
...
If a flat table format is needed (e.g. for exporting to excel etc), the 
\Rfunction{flatten} method is provided:

<<mzid3>>=
flatResults <- flatten(results)

names(flatResults)
head(flatResults)[, 1:6]
@

More information can be found in the \Rpackage{mzID} package vignette.
