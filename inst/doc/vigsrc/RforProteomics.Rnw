...
The reader is expected to have basic \R knowledge to find to document helpful. 
...
Relevant background on the \R software and its appplication to computational biology in general and proteomics in particular  
can also be found in \cite{R4prot2013}. For deatils about the Bioconductor project, the reader is refered to \cite{Gentleman2004}.
...
Since the package is not yet in the stable branch of Bioconductor, it is not yet possible to use its automatic installation and update framework unless one uses the development version of \R. An ad hoc script is however available to facilitate installation of \Rpackage{RforProteomics} and all dependencies. See \url{http://lgatto.github.com/RforProteomics/} for details. The script installs missing dependencies and then \Rpackage{RforProteomics}, which can then be loaded with
...
\begin{description}
  \item[\Rpackage{mzR}] depends on the Common Data Format\footnote{\url{http://cdf.gsfc.nasa.gov/}} (CDF) 
    to CDF-based raw mass-spectrometry data. On linux, the \texttt{libcdf} libraries are required. 
    On debian-based systems, for instance, you will need to install \texttt{netCDFlibnetcdf-dev}, 
    \texttt{netcdf-dev} and \texttt{netcdf-bin}.
  \item[\Rpackage{IPPD}] (and others) depend on the \Rpackage{XML} package which requires the \texttt{libxml2} 
    infrastructure on linux. On debian-based systems, you will need to install \texttt{libxml2-dev}.
  \item [\Rpackage{biomaRt}] performs on-line requests using the 
    \texttt{curl}\footnote{\url{http://curl.haxx.se/}} 
    infrastructure. On debian-based systems, you will need to install  
...
an \Robject{MSnSet} instance and then calculates protein intensitites 
by summing the peptide quantitation data.
Figure \ref{fig:matplot} illustates the intensities for 5 proteins.
...
nm <- NoiseModel(x)
ib.background <- subsetIBSpectra(x, protein=spks, direction = "exclude")
...
In this section, we briefly present some Bioconductor annotation infrastruture. 
...
the Gene Ontonolgy annotation package \Rpackage{GO.db} to retrieve the same 
...
Finally, this inforamtion can also be retrieved from on-line databases using the 
...
\subsection{Bioconductor pacakges}
...
The most relevant (non Bioconductor) packages are summerised below.
...
provides a GUI interface for analyzing 2D DIGE data. It allows to 
